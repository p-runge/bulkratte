...
/**
 * Some sets have a full English name in still-missing.json that differs from
 * the shorter label Pokewiki uses in its filenames.
 */
const POKEWIKI_SET_NAME_OVERRIDE: Record<string, string> = {
...
function fileBase(card: MissingCard): string {
  ...
  const setName = POKEWIKI_SET_NAME_OVERRIDE[card.setName] ?? card.setName;
  return `${name} (${setName} ${card.localId})`;
}
